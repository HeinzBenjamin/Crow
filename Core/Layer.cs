...
using Crow.Core.LearningRateFunctions;
...
namespace Crow.Core
{
    ...
    public abstract class Layer<TNeuron> : ILayer where TNeuron : INeuron
    {
        ...
        /// <summary>
        /// Sets the initial and final values for learning rate. During the learning process, the
        /// effective learning rate changes from its initial value to final value, the changing rate is defined by the chosen LearningRateFunction
        /// </summary>
        /// <param name="initialLearningRate">
        /// Initial value of learning rate
        /// </param>
        ...
        /// Final value of learning rate
        /// </param>
        /// <param name="LearningFunction"> 0: LinearFunction, 1: HyperbolicFunction, 2: ExponentialFunction</param>
        public void SetLearningRate(double initialLearningRate, double finalLearningRate, int LearningFunction)
        {
            if (LearningFunction == 1) SetLearningRate(new HyperbolicFunction(initialLearningRate, finalLearningRate));
            else if (LearningFunction == 2) SetLearningRate(new ExponentialFunction(initialLearningRate, finalLearningRate));
            else SetLearningRate(new LinearFunction(initialLearningRate, finalLearningRate));
        }
        ...
        public bool ParallelComputation { get; set; }
