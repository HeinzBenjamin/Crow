    ...
    public class ActivationNeuron : INeuron
    {
        ...
        public void Run()
        {
            if (sourceSynapses.Count > 0)
            {
                ...
                for (int i = 0; i < sourceSynapses.Count; i++)
                {
                    ...
                    double s = sourceSynapses[i].Weight * sourceSynapses[i].SourceNeuron.Output;
                    double g = s;
                }
            }
        ...
        public void RunBackward()
        {
            if (sourceSynapses.Count > 0)
            {
                ...
                for (int i = 0; i < sourceSynapses.Count; i++)
                {
                    input += sourceSynapses[i].Weight * sourceSynapses[i].SourceNeuron.Output;
                }
                input /= sourceSynapses.Count;

                //double intermediate = parent.DeactivateBackward(input);
                double intermediate = parent.Activate(input, 0.0);

                double weightSum = 0.0;
                foreach (BackpropagationSynapse t in targetSynapses)
                    weightSum += t.Weight;
                /*for(int i = 0; i < targetSynapses.Count; i++)
                    targetSynapses[i]*/
